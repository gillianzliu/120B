...
/*** some compatible functions to emulate Ardunino stuff ***/

namespace {
  inline void delay(double ms) { _delay_ms(ms); }
  ...
  void pinMode(byte pin, PinMode p) {
    if (p == OUTPUT) {
      DDRB |= (1<<pin);
    } else {
      DDRB &= ~(1<<pin);
  ...
  void digitalWrite(byte pin, BitValue v) {
    if (v == LOW) {
      PORTB &= ~(1<<pin);
    } else {
      PORTB |= (1<<pin);
  ...
  BitValue digitalRead(byte pin) {
    return (PINB & (1 << pin)) ? HIGH : LOW;
  }

  /* SPI subsystem: we simplify the Arduino library
   * a bit and keep the rest simple so that we have
   * stuff that compiles */
...
}  // namespace (for arduino compatiblity stuff)fP
